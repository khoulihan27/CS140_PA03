...
/*--------------------------------------------------------------------
 * mu_assert is a macro that returns a string if the condition expression
 * passed to it is false. When using it in a sequence of assertions,
 * a function returns a message as soon as encoutering a false condition
 * (normally it means error). The space of this message cannot be allocated on
 * the stack. It needs to be allocated with malloc or uses a global variable.
 */
