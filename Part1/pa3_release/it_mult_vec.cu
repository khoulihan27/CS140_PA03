 ...
 */
__global__ void mult_vec_async(int n, int rows_per_thread, int num_async_iter, float *y, float *d,
...
#endif
	int idx = blockId.x * threadId.x; /*Assign a linearized thread ID, so I can be responsible for some rows*/
 ...
 * Return:    void
 */
__global__ void mult_vec(int n, int rows_per_thread, float *y, float *d, float *A,
						 float *x, float *diff)
{
	int idx = blockId.x * threadId.x; /*Assign a linearized thread ID, which will be used to determine what I own*/
	/*Your solution to compute idx*/
				...
				int use_shared_x)
{
	...
	while (k < iterations)
	{
		...
		if (k < iterations)
		{ // Swap x and y pointers, so next round strts with latest solution
			float *tmp = x_d;
			x_d = y_d;
			y_d = tmp;
		}
