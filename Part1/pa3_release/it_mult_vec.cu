 ...
 * Purpose:     Run in asynchronous Gauss-Seidel style {y=d+Ay} on GPU for
 * multiple iterations. In args:     A:  matrix A d:  column vector d x:  column
 * vector x as the initial solution vector n:  the global  number of columns
 * (same as the number of rows) rows_per_thread: the number of rows for each
 * thread
 *
 * Out args:    y: final column solution vector
 */
__global__ void mult_vec_async(int n, int rows_per_thread, int num_async_iter,
                               float *y, float *d, float *A, float *x,
                               float *diff) {

#ifdef DEBUG1
  dprint_sample("GS GPU ", A, x, d, y, n, num_async_iter, !UPPER_TRIANGULAR);
#endif
  int idx = blockIdx.x * threadIdx.x; /*Assign a linearized thread ID, so I can
  ...
  for (int i = 0; i < rows_per_thread; i++) {
    int row_index = idx * rows_per_thread + i;
    diff[row_index] =
        fabs(x[row_index] - y[row_index]); // Compute the difference
  }
}
 ...
 * Return:    void
 */
__global__ void mult_vec(int n, int rows_per_thread, float *y, float *d,
                         float *A, float *x, float *diff) {
  int idx = blockIdx.x * threadIdx.x; /*Assign a linearized thread ID, which
 ...
 * Purpose:   Run t iterations of  computation:  {y=d+Ax} on the GPU, swap x and
 * y after each call. You can assume N/(num_blocks *threads_per_block) is an
 * integer In args:   A:  matrix A d:  column vector d N:  the global  number of
 * columns (same as the number of rows) t:  the number of iterations num_blocks:
 * number of blocks threads_per_block: number of threads per block In/out:    x:
 * column vector x   Contain the initial solution vector, revised iteratively y:
 * column vector y   Final solution vector diff: vector of element-wise
 * difference between x and y Return:    If return is positive, successfully
 * finish, return # of iterations executed If return is -1, it means there is at
 * least one invalid input or some execution error
 *
 */
int it_mult_vec(int N, int num_blocks, int threads_per_block, float *y,
                float *d, float *A, float *x, float *diff, int iterations,
                int use_async, int use_shared_x) {
  if (y == 0 || d == 0 || A == 0 || x == 0 || diff == 0)
    return -1;
  ...
  if (N % (num_blocks * threads_per_block)) {
    printf("The matrix size N should be divisible by "
  ...
  /*Allocate device global space for matrix A. Copy  data to the device global
   * memory*/
  /*Your solution*/
  ...
  while (k < iterations) {
    if (use_async) {
      ...
      k += NUM_ASYNC_ITER; // The above line already executes NUM_ASYNC_ITER
                           // iterations
    } else {               /* call a kernel Jacobi method to compute y=d+Ax*/
      /*Your solution*/
      mult_vec<<<num_blocks, threads_per_block>>>(N, rows_per_thread, y_d, d_d,
                                                  A_d, x_d, diff_d);
      k++;
    }
    ...
    if (k < iterations) { // Swap x and y pointers, so next round strts with
                          // latest solution
      float *tmp = x_d;
      x_d = y_d;
      y_d = tmp;
 ...
 *
 */
int it_mult_vec_seq(int N, float *y, float *d, float *A, float *x,
                    int matrix_type, int iterations) {
 ...
 *
 */
int gsit_mult_vec_seq(int N, float *y, float *d, float *A, float *x,
                      int matrix_type, int iterations) {
  int i, j, start, k, reach_converge;
 ...
 */
void print_sample(const char *msgheader, float A[], float x[], float d[],
                  float y[], int n, int t, int matrix_type) {
...
void print_samplexy(const char *msgheader, int k, float x[], float y[], int n) {
  if (k > 3 || n < 4 || x == NULL ||
      y == NULL) // No print if k is too big or n is too small
    return;
  printf("%s %d check x[0-3] %f, %f, %f, %f\n", msgheader, k, x[0], x[1], x[2],
         x[3]);
  printf("%s %d check y[0-3] %f, %f, %f, %f\n", msgheader, k, y[0], y[1], y[2],
         y[3]);
}
__device__ void dprint_samplexy(const char *msgheader, int k, float x[],
                                float y[], int n) {
